...
def resolve_fill(
	...
	Three fill-gate cases:
	  1. Empty fill side → no one is offering on the side we want to buy.
	     ...
	     (0% win rate on 82 debut-fade entries, -204c avg) because the ticker's
	...
	if not book_empty:
		...
		# debut-fade (commit fcafb11 post-mortem: 7 real fills worth +1825c/day
		# were wrongly rejected). The relative gate was belt-and-suspenders for
		# the flb-15m trade-channel bug which is already handled by Bug 1's
