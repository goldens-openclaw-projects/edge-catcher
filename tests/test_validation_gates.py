...
class TestReplaceParam:
	...
	def test_replaces_int_default(self):
		...
		out = _replace_param(self.SAMPLE, "DebutFade", "threshold_high", 72, "debut-fade__sens_72")
		params = self._reparse_params(out)
		assert params == {"threshold_high": 72, "threshold_low": 40}
		assert "debut-fade__sens_72" in out
	...
	def test_replaces_float_default(self):
		...
		out = _replace_param(code, "FlowFade", "flow_threshold", 0.575, "flow-fade__sens_0_575")
		params = self._reparse_params(out)
	...
	def test_replaces_class_and_name_even_when_values_nonmatching(self):
		...
		out = _replace_param(self.SAMPLE, "DebutFade", "threshold_high", 60, "debut-fade__sens_unchanged")
		# name attribute rewritten
		assert "name = 'debut-fade__sens_unchanged'" in out or \
			'name = "debut-fade__sens_unchanged"' in out
