...
def test_get_open_trades_for_filters(store: TradeStore) -> None:
	...
	result = store.get_open_trades_for("debut-fade", "KXBTC-25MAR-T30000")
	assert len(result) == 1
	assert result[0]["strategy"] == "debut-fade"
...
def test_save_and_load_state(store: TradeStore) -> None:
	store.save_state("debut-fade", {"last_entry": "KXBTC-T1", "count": 5})
	state = store.load_state("debut-fade")
...
def test_save_state_overwrites(store: TradeStore) -> None:
	store.save_state("debut-fade", {"count": 1})
	store.save_state("debut-fade", {"count": 99, "new_key": "hello"})
	state = store.load_state("debut-fade")
...
def test_load_all_states(store: TradeStore) -> None:
	store.save_state("debut-fade", {"a": 1})
	store.save_state("flb-15m", {"b": 2})
	all_states = store.load_all_states()
	assert "debut-fade" in all_states
	assert "flb-15m" in all_states
	assert all_states["debut-fade"]["a"] == 1
	assert all_states["flb-15m"]["b"] == 2
