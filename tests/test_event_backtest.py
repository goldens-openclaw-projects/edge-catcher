...
class StrategyD(Strategy):
	...
	def on_trade(self, trade, market, portfolio):
		...
		if trade.yes_price > self.threshold_high:
			no_price = 100 - trade.yes_price
			return [Signal(action='buy', ticker=trade.ticker, side='no', price=no_price,
				size=self.size, reason=f'debut-fade high: yes_price={trade.yes_price} > {self.threshold_high}')]
		if trade.yes_price < self.threshold_low:
			return [Signal(action='buy', ticker=trade.ticker, side='yes', price=trade.yes_price,
				size=self.size, reason=f'debut-fade low: yes_price={trade.yes_price} < {self.threshold_low}')]
		return []


class StrategyH5_15m(Strategy):
...
class TestPortfolio:
	def test_open_position_deducts_cash(self):
		...
		result = port.open_position(sig, 'sweet-spot', _dt(), slippage=0)
		assert result is True
		assert port.cash == 950.0
		assert port.has_position('T', 'sweet-spot')

	def test_open_position_slippage_increases_cost(self):
		port = Portfolio(1000.0, fee_pct=0.0)
		sig = Signal(action='buy', ticker='T', side='yes', price=50, size=1, reason='test')
		port.open_position(sig, 'sweet-spot', _dt(), slippage=2)
		assert port.cash == 948.0
		assert port.positions[('T', 'sweet-spot')].entry_price == 52
	...
	def test_open_position_returns_false_when_insufficient_cash(self):
		...
		assert not port.has_position('T', 'sweet-spot')

	def test_close_position_adds_proceeds(self):
		port = Portfolio(1000.0, fee_pct=0.0)
		sig = Signal(action='buy', ticker='T', side='yes', price=50, size=1, reason='test')
		port.open_position(sig, 'sweet-spot', _dt(), slippage=0)
		ct = port.close_position('T', 'sweet-spot', 60, _dt(1), 'take_profit', slippage=0)
	...
	def test_close_position_slippage_reduces_exit(self):
		...
		sig = Signal(action='buy', ticker='T', side='yes', price=50, size=1, reason='test')
		port.open_position(sig, 'sweet-spot', _dt(), slippage=1)  # entry = 51
		ct = port.close_position('T', 'sweet-spot', 60, _dt(1), 'take_profit', slippage=1)  # exit = 59
		assert ct is not None
	...
	def test_settle_yes_position_win(self):
		...
		port.open_position(sig, 'sweet-spot', _dt(), slippage=0)
		ct = port.settle_position('T', 'sweet-spot', 'yes', _dt(2))
		assert ct is not None
	...
	def test_settle_yes_position_loss(self):
		...
		sig = Signal(action='buy', ticker='T', side='yes', price=75, size=1, reason='test')
		port.open_position(sig, 'sweet-spot', _dt(), slippage=0)
		ct = port.settle_position('T', 'sweet-spot', 'no', _dt(2))
		assert ct is not None
	...
	def test_settle_no_position_win(self):
		...
		port.open_position(sig, 'contra-dip', _dt(), slippage=0)
		ct = port.settle_position('T', 'contra-dip', 'no', _dt(2))
		assert ct is not None
		assert ct.exit_price == 100
	...
	def test_settle_no_position_loss(self):
		port = Portfolio(1000.0)
		sig = Signal(action='buy', ticker='T', side='no', price=30, size=1, reason='test')
		port.open_position(sig, 'fade-long', _dt(), slippage=0)
		ct = port.settle_position('T', 'fade-long', 'yes', _dt(2))
		assert ct is not None
		assert ct.exit_price == 0
	...
	def test_no_fee_at_settlement(self):
		...
		port.settle_position('T', 'sweet-spot', 'yes', _dt(2))
	...
	def test_one_position_per_ticker_strategy(self):
		...
		port.open_position(sig1, 'sweet-spot', _dt(), slippage=0)
		assert port.has_position('T', 'sweet-spot')
		...
		port.open_position(sig2, 'contra-dip', _dt(), slippage=0)  # different strategy: ok
		assert port.has_position('T', 'contra-dip')
...
class TestStrategyA:
	def test_emits_buy_in_range(self):
		strategy = StrategyA(min_price=70, max_price=99)
...
class TestStrategyB:
	...
	def test_no_signal_insufficient_drop(self):
		strategy = StrategyB(drop_threshold=5)
		port = Portfolio(10000.0)
		market = _make_market()
	...
	def test_one_position_per_ticker(self):
		...
		assert len(signals) == 1
		port.open_position(signals[0], 'contra-dip', _dt(), slippage=0)
...
class TestStrategyC:
	def test_emits_buy_in_range(self):
		strategy = StrategyC(min_price=5, max_price=30)
		port = Portfolio(10000.0)
		market = _make_market()
...
class TestStrategyTP:
	def test_emits_buy_in_range(self):
		strategy = StrategyTP(min_price=40, max_price=60)
		port = Portfolio(10000.0)
		market = _make_market()
	...
	def test_emits_sell_on_take_profit(self):
		strategy = StrategyTP(min_price=40, max_price=60, take_profit=8, stop_loss=5)
		port = Portfolio(10000.0)
		market = _make_market()
...
class TestSlippage:
	def test_entry_slippage_applied(self):
		...
		port.open_position(sig, 'sweet-spot', _dt(), slippage=3)
		pos = port.positions[('T', 'sweet-spot')]
	...
	def test_exit_slippage_applied(self):
		...
		sig = Signal(action='buy', ticker='T', side='yes', price=50, size=1, reason='test')
		port.open_position(sig, 'sweet-spot', _dt(), slippage=1)  # entry=51
		ct = port.close_position('T', 'sweet-spot', 65, _dt(1), 'take_profit', slippage=2)
		assert ct is not None
...
class TestNoLookahead:
	def test_position_not_settled_before_close_time(self, tmp_path):
		...
		result = EventBacktester().run(
			series='NLSERIES',
			strategies=[StrategyA(min_price=70, max_price=99)],
			db_path=db_path,
		)
...
class TestBacktestResult:
	def _make_result(self, pnls: list[int]) -> BacktestResult:
		...
		for i, p in enumerate(pnls):
			...
			ct = CompletedTrade(
				ticker='T', side='yes', strategy='sweet-spot',
...
class TestIntegration:
	...
	def test_strategy_tp_exit_on_take_profit(self, tmp_path):
		"""StrategyTP buys YES at 50, exits at 60 (TP=8, entry+slippage=51, TP at 59)."""
		markets = [{
			'ticker': 'INT-TP',
		...
		result = EventBacktester().run(
			series='INTSERIES3',
			strategies=[StrategyTP(min_price=40, max_price=60, take_profit=8, stop_loss=5)],
			initial_cash=1000.0,
			slippage_cents=1,
			db_path=db_path,
		)
	...
	def test_strategy_tp_settles_if_no_tp_sl(self, tmp_path):
		"""StrategyTP holds to settlement if TP/SL never hit."""
		markets = [{
			'ticker': 'INT-TP2',
	...
	def test_multi_ticker_multi_strategy(self, tmp_path):
		...
		result = EventBacktester().run(
			series='MTSERIES',
			strategies=[StrategyA(), StrategyC()],
		...
		assert result.wins == 2
		assert 'sweet-spot' in result.per_strategy
		assert 'fade-long' in result.per_strategy
...
# ---------------------------------------------------------------------------
# StrategyD unit tests
# ---------------------------------------------------------------------------

class TestStrategyD:
	def test_fade_first_trade_above_threshold_buys_no(self):
		strategy = StrategyD(threshold_high=60, threshold_low=40)
		port = Portfolio(10000.0)
		market = _make_market()
	...
	def test_ignores_subsequent_trades(self):
		...
		port.open_position(signals1[0], 'debut-fade', _dt(), slippage=0)
	...
	def test_take_profit_exit_no_position(self):
		...
		strategy = StrategyD(threshold_high=60, threshold_low=40, take_profit=8, stop_loss=5)
		port = Portfolio(10000.0)
		market = _make_market()
		...
		port.open_position(buy_sig, 'debut-fade', _dt(), slippage=1)  # entry_price = 36
...
# ---------------------------------------------------------------------------
# StrategyH5_15m unit tests
# ---------------------------------------------------------------------------

class TestStrategyH5_15m:
	def test_buys_no_at_fav_threshold(self):
		strategy = StrategyH5_15m(fav_threshold=85, long_threshold=15)
		port = Portfolio(10000.0)
		market = _make_market()
	...
	def test_no_duplicate_position(self):
		...
		assert len(signals1) == 1
		port.open_position(signals1[0], 'flb-15m', _dt(), slippage=0)
	...
	def test_holds_to_settlement_no_on_market_close(self):
		...
		assert port.has_position('TEST-1', 'flb-15m')
