...
class TestResolveFill:
	...
	def test_empty_book_fallback_when_require_fresh_book_false(self) -> None:
		...
		Historically this was debut-fade's first-tick entry path. Data from
	...
	def test_empty_book_skipped_under_require_fresh_book_basic(self) -> None:
		...
		on 82 debut-fade entries, -204c avg) because the ticker-derived
	...
	def test_stale_book_fires_on_relative_drift_for_longshots(self) -> None:
		...
		pattern. Without the relative gate, flb-15m longshots silently
	...
	def test_empty_book_skipped_when_require_fresh_book(self) -> None:
		...
		Data from 2026-04-12..19 showed 82 such entries on debut-fade, all
	...
	def test_empty_book_side_asymmetric_is_skipped(self) -> None:
		...
		debut-fade's is_first_observation can fire when YES side is populated
...
class TestRiskBudgetCap:
	...
	10c tolerance). For longshot strategies like flb-15m that enter at
